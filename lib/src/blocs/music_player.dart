...
class MusicPlayerBloc {
  late BehaviorSubject<List<Song>> _songs$;
  late BehaviorSubject<List<Album>> _albums$;
  late BehaviorSubject<MapEntry<PlayerState, Song>> _playerState$;
  late BehaviorSubject<MapEntry<List<Song>, List<Song>>>
  ...
  late BehaviorSubject<List<Song>> _favorites$;
  late BehaviorSubject<bool> _isAudioSeeking$;
  late AudioPlayer _audioPlayer;
  late Song _defaultSong;
  ...
  Future<void> fetchSongs() async {
    await MusicHelper.getAllSongs().then((value) => _songs$.add(value
        .map((e) => Song(
              e.id,
              e.artist ?? TextHelper.notAvailable,
              e.title,
              e.album ?? TextHelper.notAvailable,
              e.albumId ?? 0,
              e.duration ?? 0,
              e.data,
              e.uri ?? TextHelper.notAvailable,
            ))
        .toList()));
  ...
  void playMusic(Song song) {
    log('Song Details: $song');
    // TODO check sources
    _audioPlayer.play(DeviceFileSource(song.uri));
  ...
  void updateDuration(Duration duration) {
    _audioPlayer.seek(Duration(milliseconds: duration.inMilliseconds));
  }
  ...
  void audioSeek(double seconds) {
    _audioPlayer.seek(
        Duration(milliseconds: 1000 * int.parse(seconds.toStringAsFixed(3))));
  }
  ...
  void _initAudioPlayer() {
    ...
    _audioPlayer.onPositionChanged.listen((event) {
      ...
      if (!_isAudioSeeking) {
        updatePosition(Duration(seconds: event.inSeconds));
      }
    });
